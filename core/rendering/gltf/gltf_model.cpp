...
namespace rvr {
GLTFModel::GLTFModel(std::shared_ptr<RenderingContext> renderingContext, const std::string& fileName) :
...
void GLTFModel::LoadGLTFFile(const std::string& fileName) {
    ...
    auto vertexStagingBuffer = Buffer(renderingContext_, sizeof(gltf::Vertex),
                                      gltfVertexBuffer.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                      MemoryType::HostVisible);
    auto indexStagingBuffer = Buffer(renderingContext_, sizeof(uint32_t),
    ...
    auto vertexBuffer = std::make_unique<Buffer>(renderingContext_, sizeof(gltf::Vertex),
                                                   gltfVertexBuffer.size(),
                                                   VK_BUFFER_USAGE_VERTEX_BUFFER_BIT |
                                                   VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                                   MemoryType::DeviceLocal);
    auto indexBuffer = std::make_unique<Buffer>(renderingContext_, sizeof(uint32_t),
...
void GLTFModel::DrawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, gltf::Node* node) {
...
void GLTFModel::Draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout) {
    ...
    for (auto& transform : pushConstantTransforms_) {
        ...
        for (gltf::Node* node : nodes_) {
            // set top level node the incoming transform
...
uint32_t GLTFModel::GetNumImages() {
    return static_cast<uint32_t>(images_.size());
}

std::vector<gltf::Image>& GLTFModel::GetImages() {
    return images_;
}

void GLTFModel::AddPushConstant(glm::mat4 transform) {
    pushConstantTransforms_.push_back(transform);
}

void GLTFModel::ClearPushConstants() {
